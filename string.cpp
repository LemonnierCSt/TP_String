...
string::string(const char* chain){
  ...
  else { //If the number of characters is bigger than the maximum
...
string operator+(const string& lhs, const string& rhs) { 
  if(lhs.size_+rhs.size_>lhs.MAX_SIZE){ // if the concatenation's size_ is greater than MAX_SIZE, we only return the lhs argument 
    ...
    char* newData=new char[lhs.size_+rhs.size_ +1]; //Creating a char* the right size
    for(size_t i=0;i<lhs.size_;++i){
      newData[i]=lhs.data_[i];
    }
    for(size_t j=0;j<=rhs.size_;++j){
      newData[j+lhs.size_]=rhs.data_[j];//copies the second string, including the '\0' 
    }
